...
#define alog(fmt, args...) __android_log_print(ANDROID_LOG_DEBUG, "jsengine", fmt, ##args);
...
static jsval_t call_js(struct js *js, const char *fn, int fnlen) {
  ...
  while (fnpos < fnlen) {
    ...
    //alog("  [%.*s] -> %u [%.*s] -> ", (int) identlen, &fn[fnpos], js->pos,
    //(int) js->clen, js->code);
